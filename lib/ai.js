...
export async function enrichSite({ name, domain, url, screenshotUrl, vocabulary, discoveredPages = [] }) {
  ...
  const pagesBlock = discoveredPages.length
    ? `\n\nThis site also has these nav/footer links discovered on the homepage. For EACH one, classify its page_type using ONLY these values (or null if genuinely none fit): ${vocabulary.page_type.join(", ")}. Return them in the "pages" field as [{url, page_type}], one entry per link below, using the exact url given — don't alter it.\n${discoveredPages.map((p) => `- ${p.url} ("${p.label}")`).join("\n")}`
    : "";
