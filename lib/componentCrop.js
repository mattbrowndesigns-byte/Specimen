...
export async function enrichAndSaveComponent({ componentId, sourceUrl, croppedBuffer, clearExistingTags }) {
  ...
  let result;
  try {
    result = await enrichComponent({
      sourceUrl,
      imageBase64: croppedBuffer.toString("base64"),
      vocabulary,
    });
  } catch (err) {
    console.error("Component enrichment failed:", err.message);
    return;
  }
  ...
  if (tagIdsToLink.length) {
    ...
    await supabase.from("taggable").upsert(rows, { onConflict: "tag_id,target_type,target_id" });
  }
