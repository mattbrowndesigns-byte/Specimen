...
export async function attachSiteFavicons(supabase, components) {
  ...
  if (!siteIds.length) return components.map((c) => ({ ...c, favicon_url: null, favicon_fills: true }));

  const { data: sites, error } = await supabase
    .from("site")
    .select("id, favicon_url, favicon_fills")
    .in("id", siteIds);
  if (error) throw new Error(error.message);

  const byId = new Map((sites || []).map((s) => [s.id, s]));
  return components.map((c) => {
    const site = byId.get(c.site_id);
    return { ...c, favicon_url: site?.favicon_url || null, favicon_fills: site?.favicon_fills ?? true };
  });
}
