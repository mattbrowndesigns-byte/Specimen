...
export async function runEnrichment(siteId) {
  ...
  if (discoveredPages?.length && Array.isArray(result.pages) && result.pages.length) {
    ...
    for (const entry of result.pages) {
      ...
      const utilityLabel =
        typeof entry.utility_label === "string" && entry.utility_label.trim()
          ...
          : null;
      selected.push({ id: page.id, pageType, utilityLabel });
    }
    ...
    if (selected.length) {
      ...
      for (const { id, pageType, utilityLabel } of selected) {
        const { error: pageUpdateError } = await supabase
          .from("page")
          .update({ is_representative: true, page_type: pageType, utility_label: utilityLabel })
          .eq("id", id);
        if (pageUpdateError) {
