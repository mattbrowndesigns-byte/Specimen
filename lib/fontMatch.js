...
export async function describeFonts({ domain, fonts, fontHosts = [], probes = {} }) {
  ...
  let identified = [];
  try {
    const result = await identifyFonts({ domain, fonts, fontHosts, probes });
  ...
  return Promise.all(
    base.map(async (font) => {
      ...
      // `described` means one thing: the Google and Adobe checks ran on this
      // face. It's what lets the panel tell "we looked and there's nothing
      // close" from "nobody has looked yet", so it goes only here -- not on a
      // row the model declined to name, and not on the ones handed back when
      // the identify call fails, because neither of those was checked.
      const row = {
        ...font,
        described: true,
