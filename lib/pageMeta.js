...
//
// A colon needs no space in front of it -- "Littlebird: AI Assistant With a
// Memory of Your Work" is the common shape, and requiring one left the whole
// tagline in the name. A hyphen still does, so hyphenated brands survive.
...
// The resource variant: one fetch, no link extraction. A resource has no
// detail page and no page discovery, so the nav/footer walk that `fetchPageMeta`
// does for a site is pure cost here.
//
// The title is cleaned the same way a site's is, because a tool's <title> is
// built like a company's -- "Lucide | Beautiful & consistent icons" -- and the
// brand half is the half worth keeping. The tagline isn't thrown away: it goes
// to the model as the description, which is often a better one-liner than
// anything generated from the body text.
export async function fetchResourceMeta(url) {
  const empty = { title: null, description: null, textSnippet: null, faviconUrl: null };
  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 8000);
    const res = await fetch(url, {
    ...
    const rawTitle = $('meta[property="og:title"]').attr("content") || $("title").first().text().trim();
    const description =
    ...
    return {
      title: cleanSiteName(rawTitle, domain) || rawTitle || null,
      description: description?.trim().slice(0, 400) || null,
      textSnippet: $("body").text().replace(/\s+/g, " ").trim().slice(0, 1000) || null,
      faviconUrl: extractFavicon($, url),
    };
  } catch (err) {
    console.error("Failed to fetch resource metadata:", err.message);
    return empty;
  }
}
