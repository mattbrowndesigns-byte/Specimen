...
function extractLinks($, baseUrl, selfUrl) {
  ...
  // nav and footer are gathered separately so a long mega-menu can't starve
  // the footer, which is where the evergreen pages usually are.
  for (const scope of ["nav a[href]", "footer a[href]"]) {
    $(scope).each((_, el) => {
